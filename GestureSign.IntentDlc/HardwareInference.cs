...
internal sealed class HardwareInference : IDisposable
{
    ...
    private IntentModel? _model; private bool _hybrid;
    ...
    private static async Task PrepareProvidersAsync(bool install)
    {
        await ProviderGate.WaitAsync();
        try
        {
            AmdNpuBackend.Register();
            foreach (var provider in ExecutionProviderCatalog.GetDefault().FindAllProviders())
            {
                try
                {
                    if (provider.Certification != ExecutionProviderCertification.Certified) continue;
                    if (provider.ReadyState == ExecutionProviderReadyState.NotPresent && !install) continue;
                    ...
                    if (provider.Name == "VitisAIExecutionProvider")
                        AmdNpuBackend.Register(provider.LibraryPath);
                    else
                        provider.TryRegister();
                }
                catch (Exception ex) { DiagnosticTrace?.Invoke(provider.Name + ": " + ex.Message); }
            }
        }
    ...
    {
        try { await PrepareProvidersAsync(false); UpdateHardwarePreview(); }
    ...
    public async Task LoadAsync(IntentModel model, bool installProviders, string? testVendor = null)
    {
        ...
        model.Validate();
        try { await PrepareProvidersAsync(installProviders); }
        ...
        await Task.Run(() =>
        {
            lock (_sync)
            {
                ...
                try
                {
                    AmdNpuBackend.Register(); var env = OrtEnv.Instance();
                    ...
                    foreach (var kind in new[] { OrtHardwareDeviceType.NPU, OrtHardwareDeviceType.GPU })
                        foreach (var device in devices.Where(d => d.HardwareDevice.Type == kind && (testVendor == null || d.HardwareDevice.Vendor.Contains(testVendor, StringComparison.OrdinalIgnoreCase))))
                        {
                            ...
                            if (kind == OrtHardwareDeviceType.NPU && device.EpName == "VitisAIExecutionProvider" && AmdNpuBackend.Registered)
                            {
                                _fallbacks.Enqueue(("NPU · AMD VitisAI（线性计算）+ CPU（归一化 / Sigmoid / 精度复核）", true, () => AmdNpuBackend.Create(model, device)));
                                continue;
                            }
                            _fallbacks.Enqueue(($"{kind} · {device.EpName} · {device.HardwareDevice.Vendor}", false, () =>
                            {
                ...
                _fallbacks.Enqueue(("CPU · ONNX Runtime", false, () =>
                {
    ...
    private void Advance()
    {
        while (_fallbacks.TryDequeue(out var backend))
        {
            try
            {
                ...
                if (Accept(session, _model!, backend.Hybrid)) { _hybrid = backend.Hybrid; _session = session; _backend = backend.Name; return; }
            }
    ...
    private static bool Accept(InferenceSession session, IntentModel model, bool hybrid)
    {
        try
        {
            foreach (var offset in new[] { -1f, 0f, 1f })
            {
                ...
                float score = hybrid ? AmdNpuBackend.Run(session, model, input) : Run(session, input);
    ...
    public IntentPrediction Predict(float[] features)
    {
        lock (_sync)
        {
            ...
            while (_session != null)
            {
                try
                {
                    float score = _hybrid ? AmdNpuBackend.Run(_session, _model, features) : Run(_session, features);
