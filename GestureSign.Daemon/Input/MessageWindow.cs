...
namespace GestureSign.Daemon.Input
{
    public class MessageWindow : NativeWindow
    {
        ...
        private void RegisterDevice(ushort usage)
        {
            ...
            {
                Logging.LogMessage($"Raw input registration failed. Usage=0x{usage:X}, Error={Marshal.GetLastWin32Error()}");
                throw new ApplicationException("Failed to register raw input device(s).");
            }
            _registeredDeviceList.Add(usage);
            Logging.LogMessage($"Raw input registered. Usage=0x{usage:X}, Hwnd={Handle}");
        }
        ...
        private bool ValidateDevice(IntPtr hDevice, out ushort usage)
        {
            ...
            using (new SafeUnmanagedMemoryHandle(pInfo))
            {
                ...
                using (new SafeUnmanagedMemoryHandle(pData))
                {
                    ...
                    if (info.hid.usUsage != NativeMethods.TouchPadUsage &&
                        (string.IsNullOrEmpty(deviceName) || deviceName.IndexOf("VIRTUAL_DIGITIZER", StringComparison.OrdinalIgnoreCase) >= 0 || deviceName.IndexOf("ROOT", StringComparison.OrdinalIgnoreCase) >= 0))
                        return true;
            ...
            try
            {
                ...
                else if (usage == NativeMethods.TouchPadUsage)
                {
                    ...
                    using (TouchPadDevice touchPad = new TouchPadDevice(buffer, ref raw))
                    {
                        ...
                        touchPad.GetRawDatas(linkCollection[0].NumberOfChildren, _currentScr, ref _requiringContactCount, ref _outputTouchs, contactCount != 0);
                    }
                }
